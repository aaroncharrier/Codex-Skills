...
Use this file as the canonical reference for input requirements, output schemas, scope boundaries, issue classification, severity rules, and handoff behavior. Do not quote it in the final response. Apply it.
...
- determine pass/fail and rework requirement
...
2. `LOGICAL`
   - step mismatch with execution plan
   - incorrect ordering
...
4. `CONSISTENCY`
   - spec vs artifact mismatch
   - plan vs artifact mismatch
5. `RISK`
   - ambiguous logic
   - fragile assumptions
